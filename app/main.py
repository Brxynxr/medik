...
from fastapi import FastAPI, Request, File, Form, UploadFile, HTTPException, status, Response, BackgroundTasks
from fastapi.responses import HTMLResponse, JSONResponse, StreamingResponse
from fastapi.staticfiles import StaticFiles
from fastapi.templating import Jinja2Templates

from app.services.ocr_service import warmup_document_ocr
...
def _infer_document_routing_and_renaming(findings: List[HallazgoEnriquecido], total_pages: int, pdf_hash: str) -> Tuple[str, str, str]:
    """Infiere el departamento corporativo responsable, el nombre estandarizado y el resumen ejecutivo."""
    from datetime import datetime
    import re

    finding_map = {f.parametro.lower(): f.valor for f in findings if f.valor and f.valor != "No detectado en el documento"}

    # 1. Detectar fecha
    raw_date = finding_map.get("fecha") or datetime.now().strftime("%Y-%m-%d")
    date_clean = re.sub(r"[^\d]", "", raw_date)[:8]
    if len(date_clean) < 8:
        date_clean = datetime.now().strftime("%Y%m%d")

    # 2. Detectar ID / NIT
    doc_id = finding_map.get("nit") or finding_map.get("cedula") or finding_map.get("identificacion") or pdf_hash[:8].upper()
    doc_id_clean = re.sub(r"[^\w]", "", doc_id)[:12]

    # 3. Categorizar departamento
    if any(k in finding_map for k in ["total", "valor total", "subtotal", "iva", "factura"]):
        category = "FACTURA"
        dept = "Contabilidad / Cuentas por Pagar"
    elif any(k in finding_map for k in ["diagnostico", "paciente", "medico", "historia clinica", "clinica"]):
        category = "HISTORIA_CLINICA"
        dept = "Salud / Asistencia Médica"
    elif any(k in finding_map for k in ["arrendador", "arrendatario", "clausula", "contrato", "vigencia"]):
        category = "CONTRATO"
        dept = "Jurídico / Gestión Contractual"
    elif any(k in finding_map for k in ["reclamo", "peticion", "queja", "solicitud"]):
        category = "PQRS"
        dept = "Servicio al Cliente / PQRS"
    else:
        category = "DOCUMENTO"
        dept = "Administración General"

    suggested_filename = f"{date_clean}_{category}_{doc_id_clean}.pdf"
    summary = f"Expediente de {total_pages} página(s) clasificado como {category} con destino a {dept}."
    return dept, suggested_filename, summary
...
def _try_auto_process_file(pdf_hash: str) -> Optional[JobOutput]:
    ...
    sample_docs_dir = Path(BASE_DIR).parent / "tests" / "sample_docs"
    if sample_docs_dir.exists():
        candidates.extend(sample_docs_dir.glob("*.pdf"))
...
async def health_check():
    ...
    return {
        "status": "healthy",
        "app": "SmartDoc Engine",
        "brand": "SmartDoc Engine — Buzón Corporativo RIWI",
        "alias": "PyDective",
        "version": "3.0.0",
...
async def procesar_documento(
    ...
    # 3. Consulta temprana de Caché L0 instantánea ultra-rápida (US-14)
    cached_l0 = get_l0_cache(raw_hash, query_hash)
    if cached_l0 is not None:
        MOCK_RESULTS_STORE[raw_hash] = cached_l0
        return cached_l0

    # 4. Reutilización temprana de Conocimiento en Caché L1 documental (US-15)
    cached_l1 = get_l1_cache(raw_hash)
    if cached_l1 is not None and cached_l1.status == EstadoCobertura.COMPLETE:
        resolved_l1 = resolve_from_l1(cached_l1, canonical_params, raw_hash, query_hash)
        if resolved_l1 is not None:
            MOCK_RESULTS_STORE[raw_hash] = resolved_l1
            return resolved_l1
    ...
    async def _do_extraction():
        ...
        try:
            ...
            for p_idx in range(total_pages):
                ...
                # Catalogación física y forense de imágenes (US-13) - Reutiliza metadatos pre-extraídos
                page_visuals = []
                if catalogar_imagenes:
                    page_visuals = catalog_page_images(
                        page,
                        catalogar_imagenes=True,
                        pre_extracted_visuals=classification.metadatos_visuales,
                    )
            ...
            # Inferencia multimodal con Gemini en carril NEEDS_AI (US-11, US-12, US-18)
            # Solo enviar a inferencia si existen parámetros pendientes no resueltos nativamente
            found_params = {h.parametro for h in all_spatial_findings if _eval_finding_quality(h) >= 0.70}
            missing_params = [p for p in canonical_params if p not in found_params]

            if ai_queue and missing_params:
                target_ai_queue = ai_queue[:3]
                ...
                max_workers = min(settings.MAX_WORKERS_PER_JOB, len(target_ai_queue))
                with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as pool:
                    tasks = [loop.run_in_executor(pool, _invoke_ai, item) for item in target_ai_queue]
        ...
        dept_sug, file_sug, sum_sug = _infer_document_routing_and_renaming(final_hallazgos, total_pages, pdf_hash)
        ...
        output = JobOutput(
            ...
            departamento_sugerido=dept_sug,
            nombre_archivo_sugerido=file_sug,
            resumen_documento=sum_sug,
...
async def procesar_documento_stream(
    ...
    if cached_l0 is not None:
        async def cached_stream_gen():
            yield f"data: {json.dumps({'tipo': 'inicio', 'pdf_hash': raw_hash, 'total_paginas': cached_l0.paginas_totales, 'nivel_cache': 'L0'})}\n\n"
            ...
            yield f"data: {json.dumps({'tipo': 'completado', 'pdf_hash': raw_hash, 'status': cached_l0.status.value, 'nivel_cache': 'L0', 'duracion_total_ms': cached_l0.duracion_total_ms, 'paginas_totales': cached_l0.paginas_totales, 'paginas_pendientes': cached_l0.paginas_pendientes, 'hallazgos': [h.model_dump() for h in cached_l0.hallazgos]})}\n\n"
    ...
    if cached_l1 is not None and cached_l1.status == EstadoCobertura.COMPLETE:
        ...
        if resolved_l1 is not None:
            async def l1_stream_gen():
                yield f"data: {json.dumps({'tipo': 'inicio', 'pdf_hash': raw_hash, 'total_paginas': resolved_l1.paginas_totales, 'nivel_cache': 'L1'})}\n\n"
                ...
                yield f"data: {json.dumps({'tipo': 'completado', 'pdf_hash': raw_hash, 'status': resolved_l1.status.value, 'nivel_cache': 'L1', 'duracion_total_ms': resolved_l1.duracion_total_ms, 'paginas_totales': resolved_l1.paginas_totales, 'paginas_pendientes': resolved_l1.paginas_pendientes, 'hallazgos': [h.model_dump() for h in resolved_l1.hallazgos]})}\n\n"
    ...
    async def event_generator():
        try:
            ...
            for p_idx in range(total_pages):
                ...
                # 2. Inferencia multimodal con Gemini en carril NEEDS_AI si faltan parámetros
                found_params = {h.parametro for h in all_spatial_findings if _eval_finding_quality(h) >= 0.70}
                missing_params = [p for p in canonical_params if p not in found_params]

                if classification.tipo == TipoPagina.NEEDS_AI and missing_params:
                    t_ren = time.perf_counter()
                    webp_bytes = get_or_render_page_webp(page, pdf_hash, p_num, max_dim=1024, quality=75)
                    total_render_ms += (time.perf_counter() - t_ren) * 1000
                    ...
                    except (asyncio.TimeoutError, Exception) as gem_err:
...
@app.get("/documentos/{pdf_hash}/download")
async def descargar_documento_estandarizado(pdf_hash: str):
    """Descarga el documento con su nombre corporativo normalizado ({FECHA}_{CATEGORIA}_{ID}.pdf)."""
    pdf_bytes = get_pdf_bytes_by_hash(pdf_hash)
    if not pdf_bytes:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"El archivo PDF con hash '{pdf_hash}' no fue encontrado.",
        )
    resultado = get_job_result(pdf_hash)
    filename = (resultado.nombre_archivo_sugerido if resultado and resultado.nombre_archivo_sugerido else f"{pdf_hash[:16]}.pdf")
    return Response(
        content=pdf_bytes,
        media_type="application/pdf",
        headers={
...
@app.get("/exportar/{pdf_hash}/excel")
async def exportar_auditoria_excel(pdf_hash: str):
    """Genera y descarga un reporte formal de auditoría en formato Excel (.xlsx)."""
    resultado = get_job_result(pdf_hash)
    if not resultado:
        resultado = _try_auto_process_file(pdf_hash)
    if not resultado:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Documento '{pdf_hash}' no encontrado.",
        )
    ...
    for h in resultado.hallazgos:
        records.append({
            "SHA-256": resultado.pdf_hash,
            "Departamento Destino": resultado.departamento_sugerido or "General",
            "Nombre Estandarizado": resultado.nombre_archivo_sugerido or "N/A",
            "Parámetro": h.parametro,
            "Valor Extraído": h.valor,
            "Confianza (%)": round(h.confianza * 100, 1),
            "Método Extracción": h.metodo.value if hasattr(h.metodo, 'value') else str(h.metodo),
            "Páginas con Evidencia": ", ".join(str(e.page) for e in h.evidencias) if h.evidencias else "N/A",
            "Duración Total (ms)": resultado.duracion_total_ms,
            "Nivel Caché": resultado.nivel_cache.value if hasattr(resultado.nivel_cache, 'value') else str(resultado.nivel_cache),
        })

    df = pd.DataFrame(records)
    out_buf = io.BytesIO()
    with pd.ExcelWriter(out_buf, engine="openpyxl") as writer:
        df.to_excel(writer, index=False, sheet_name="Auditoria_Documental")
    out_buf.seek(0)

    filename = f"auditoria_{resultado.pdf_hash[:8]}.xlsx"
    return Response(
...
@app.get("/exportar/{pdf_hash}/csv")
async def exportar_auditoria_csv(pdf_hash: str):
    """Genera y descarga un reporte formal de auditoría en formato CSV."""
    resultado = get_job_result(pdf_hash)
    if not resultado:
        resultado = _try_auto_process_file(pdf_hash)
    if not resultado:
    ...
    for h in resultado.hallazgos:
        records.append({
            "SHA-256": resultado.pdf_hash,
            "Departamento": resultado.departamento_sugerido or "General",
            "Nombre Estandarizado": resultado.nombre_archivo_sugerido or "N/A",
            "Parámetro": h.parametro,
            "Valor Extraído": h.valor,
            "Confianza (%)": round(h.confianza * 100, 1),
            "Método": h.metodo.value if hasattr(h.metodo, 'value') else str(h.metodo),
            "Evidencias": ", ".join(str(e.page) for e in h.evidencias) if h.evidencias else "N/A",
        })

    df = pd.DataFrame(records)
    csv_bytes = df.to_csv(index=False).encode("utf-8")
    filename = f"auditoria_{resultado.pdf_hash[:8]}.csv"
    return Response(
        content=csv_bytes,
        media_type="text/csv",
        headers={"Content-Disposition": f'attachment; filename="{filename}"'},
