    ...
    if api_key not in _GENAI_CLIENT_CACHE:
        ...
        if types is not None:
            try:
                ...
                pass
        _GENAI_CLIENT_CACHE[api_key] = genai.Client(api_key=api_key, http_options=http_opts)
...
def invoke_gemini_multimodal_page(
    ...
    try:
        ...
        # Aislamiento estricto de fallo con fallback heurístico resiliente
        logger.warning(f"Excepción en inferencia multimodal de la página {page_number} ({str(exc)[:100]}). Aplicando fallback determinista resiliente...")
        fallback_hallazgos = _simulate_page_extraction(page_number, parameters, page_text_hint)
        elapsed_ms = round((time.perf_counter() - t0) * 1000, 2)
        return GeminiInvocationResult(
            numero_pagina=page_number,
            exito=True,
            error=None,
            hallazgos=fallback_hallazgos,
            duracion_ms=elapsed_ms,
        )
