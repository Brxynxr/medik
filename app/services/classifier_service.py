...
def classify_page(
    ...
    # 7. BYPASS DETERMINISTA NATIVO (US-04 Escenario 1 - Máxima Prioridad de Velocidad)
    # Si la página tiene texto digital suficiente y legible, procesar 100% en Cero-IA (PyMuPDF C-speed)
    if word_count >= 40 and readability >= min_readability_score:
        return PageClassification(
            numero_pagina=page_num,
            ...
            total_graphics_area_ratio=total_graphics_area_ratio,
            bypass_opencv=True,
            motivo=f"Texto digital nativo suficiente ({word_count} palabras >= 40) y legible ({readability:.2f}).",
        )

    # 8. Texto breve pero estructurado y limpio (US-05 Escenario 2)
    if readability >= 0.70 and word_count >= 20:
        return PageClassification(
            numero_pagina=page_num,
            tipo=TipoPagina.LOCAL,
            readability_score=readability,
    ...
    # 9. Páginas con muy poco texto (< 20 palabras) y dominadas por gráficos o escaneos
    if total_graphics_area_ratio >= 0.20 or word_count < 20:
        return PageClassification(
            numero_pagina=page_num,
            tipo=TipoPagina.NEEDS_AI,
            readability_score=readability,
            ...
            total_graphics_area_ratio=total_graphics_area_ratio,
            bypass_opencv=False,
            motivo=f"Página con elementos gráficos dominantes y texto escaso ({word_count} palabras, área gráfica={total_graphics_area_ratio:.2f}).",
        )

    # 10. Por defecto carril LOCAL rápido para páginas textuales
    return PageClassification(
        numero_pagina=page_num,
        tipo=TipoPagina.LOCAL,
        readability_score=readability,
        ...
        total_graphics_area_ratio=total_graphics_area_ratio,
        bypass_opencv=True,
        motivo="Página textual digital procesable localmente.",
    )
