/**
 * SmartDoc Engine — PDF Viewer Controller (PDF.js Integration)
 * Proporciona:
...
class SmartDocPdfViewer {
    constructor(options = {}) {
        ...
        this.pageInput = document.getElementById(options.pageInputId || 'pdf-page-current-input');
        ...
        this.totalPages = 0;
        this.currentPage = 1;
        this.scale = options.initialScale !== undefined ? options.initialScale : 0.8;
        ...
        if (this.zoomLevelSpan) {
            this.zoomLevelSpan.innerText = `${Math.round(this.scale * 100)}%`;
        }
    ...
    async loadDocument() {
        ...
        try {
            ...
            console.error('[SmartDoc Viewer] Error cargando PDF:', err);
            this.container.innerHTML = `
    ...
    async renderPageCanvas(pageNum, canvas, overlay) {
        try {
            ...
            console.warn(`[SmartDoc Viewer] Error renderizando página ${pageNum}:`, e);
        }
    }
    ...
    async executeSearch(query) {
        ...
        try {
            ...
            console.error('[SmartDoc Viewer] Error en búsqueda exacta:', err);
        }
    }
    ...
    renderAllSearchHighlights() {
        ...
        this.matches.forEach((m, idx) => {
            ...
            box.className = 'pdf-search-match-box pdf-highlight-match';
    ...
    focusCurrentMatch() {
        ...
        document.querySelectorAll('.pdf-search-match-box, .pdf-highlight-match').forEach(el => {
        ...
        if (activeBox) {
            ...
            activeBox.classList.add('active-match');
        }
    ...
    highlightSource(pageNum, bbox, label = 'Evidencia Documental') {
        ...
        const oldG = document.querySelectorAll('.pdf-grounding-target, .pdf-grounding-bbox');
        ...
        if (Array.isArray(bbox) && bbox.length === 4) {
            ...
            gBox.className = 'pdf-grounding-bbox pdf-grounding-target animate-pulse-glow';
            ...
            badge.className = 'pdf-grounding-tooltip pdf-grounding-label';
...
window.SmartDocPdfViewer = SmartDocPdfViewer;
window.PydectivePdfViewer = SmartDocPdfViewer;
...
window.highlightSourceInPdf = function(pageNum, bbox, label) {
    if (window.activePdfViewer) {
        ...
        console.warn('[SmartDoc Engine] Visor de PDF no inicializado aún.');
    }
};
