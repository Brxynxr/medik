...
class JobOutput(BaseModel):
    ...
    resumen_documento: Optional[str] = None
    departamento_sugerido: Optional[str] = None
    nombre_archivo_sugerido: Optional[str] = None
...
class ChatMessage(BaseModel):
    role: str = Field(default="user", description="'user' o 'assistant'")
    ...
    def handle_aliases(cls, data: Any) -> Any:
        if isinstance(data, dict):
            ...
            content = data.get("content") or data.get("mensaje") or data.get("text") or ""
            timestamp = data.get("timestamp") or data.get("fecha")
